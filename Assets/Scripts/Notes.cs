...
public class Notes : MonoBehaviour {
    ...
    public void Create(NotesModel nm,float speed)
    {
        if (nm.type == 0 || nm.type == 1)
        {
            ...
            int len = nm.end - nm.start + 1;
            Vector3 v = gameObject.transform.position;
            v.x = (nm.start + len / 2) - 8;
        ...
        //    transform.localScale = new Vector3(nm.size - 0.2f, 0.01f, 1);
            ...
            if (nm.type == 2)
            {
                mesh.vertices = new Vector3[] {
                    ...
                    new Vector3 (nm.end - 7.2f, 0, nm.hold * speed * 1.5f),
                };
                mesh.uv = new Vector2[] {
                    new Vector2 (0, 0),
                    new Vector2 (1f, 0),
            ...
            if (nm.type == 3)
            {
                ...
                float f = 1f / (nm.slide.Count);
                
                Vector2[] uv = new Vector2[(nm.slide.Count + 1) * 2];
                int iii = 0;
                for (int j = 0;j < uv.Length  ; j++)
                {
                    uv[j] = new Vector2(0,f * iii);
                    uv[++j] = new Vector2(1, f * iii);
                    iii++;
                }
